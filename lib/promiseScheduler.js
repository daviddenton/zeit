...
exports.PromiseScheduler = function (clock) {
    ...
    function createPromiseCallbackFn(item, state, onDone) {
        return function () {
            if (item.invocationsLeft === 0 || !state.prePredicate()) {
                onDone();
                return;
            }
            ...
            q.resolve().then(state.promiseFn).done(function (result) {
    ...
    var addSchedule = function (state) {
        ...
        var callback = createPromiseCallbackFn(item, state, function (err, result) {
            if (!state.prePredicate() || !state.postPredicate(err, result) || item.invocationsLeft === 0 || (!item.invocationsLeft && !item.repeatInterval)) {
