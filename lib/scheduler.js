...
function ScheduleItemBuilder(callbackFn, startCallback) {
    var state = {
...
exports.Scheduler = function (clock) {
    ...
    var addSchedule = function (state) {
        ...
        var callback = createPromiseCallbackFn(item, state, function (err, result) {
            if (!state.prePredicate(item) ||
                state.postPredicate(err, result) ||
                item.invocationsLeft === 0 ||
                (!item.invocationsLeft && (item.repeatInterval === undefined))) {
    ...
    events.execute = function (callbackFn) {
        return new ScheduleItemBuilder(callbackFn, addSchedule);
    };
