...
exports.PromiseScheduler = function (clock) {
    ...
    events.schedule = function (promiseFn, duration, name) {
        ...
        activeSchedules[scheduleId] = {
            name: name,
            clockId: clock.setTimeout(createPromiseCallbackFn(scheduleId, promiseFn, _.noop), clock.durationToMilliseconds(duration))
        };
        return scheduleId;
    };
    ...
    events.scheduleEvery = function (promiseFn, duration, name) {
        ...
        var callback = createPromiseCallbackFn(scheduleId, promiseFn, function () {
            activeSchedules[scheduleId].clockId = clock.setTimeout(callback, clock.durationToMilliseconds(duration));
        });
